...
int stptr,tnum,onum;
void push_stack(), pop_stack(),write_stack(),three_num_code();
int comp_token();
...
int main()
{
	...
	for(i=0;i<tnum;i++){
		...
		if(token[i][0]>='A' && token[i][0]<='Z'){  //トークンが変数の場合 
			...
			push_stack(token[i]);	//stackにプッシュダウン
		}else if(strcmp(token[i],")")==0){
			while( strcmp(stack[stptr-1],"(" )!= 0){
				strcpy(outstring[onum],stack[stptr-1]);
				onum++;
				pop_stack();	//stackの先頭を削除
			}
			pop_stack();	//stackの先頭に"("がくるので削除
		}else{
...
three_num_code()
{
	...
	for(i=0;i<onum;i++){
		if((outstring[i][0]>='A' && outstring[i][0]<='Z')){ //変数ならば
			...
			pop_stack();
			strcpy(threecode[1],stack[stptr-1]);
			pop_stack();
			...
			printf("LD $%s\n",threecode[1]);	
			if(strcmp(threecode[0],"+") == 0){	// +のとき
				printf("AD $%s\n",threecode[2]);
			}else if(strcmp(threecode[0],"-") == 0){	//　-のとき
				printf("SB $%s\n",threecode[2]);
			}else if(strcmp(threecode[0],"*") == 0){	//　*のとき
				printf("ML $%s\n",threecode[2]);	
			}else if(strcmp(threecode[0],"/") == 0){	//　/のとき
			...
			strcpy(twocode[1],stack[stptr-1]);
			pop_stack();
			ss[0]=c; ss[1]='\0';
			push_stack(ss);
			strcpy(twocode[2],ss);
